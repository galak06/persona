...
def main() -> None:
    ...
    for task in schedule.get("tasks", []):
        ...
        sched  = f"Daily {sched}" if "Daily" not in task["schedule"]["human"] else task["schedule"]["human"]

        # Extra stat
        stat = ""
        if key == "fb_scanner":
            stat = f"groups={entry.get('groups_scanned','?')} queued={entry.get('posts_queued','?')}"
        elif key == "ig_scanner":
    ...
    skipped        = [q for q in queue if q.get("status") in ("USER_SKIPPED", "VALIDATION_FAILED", "skipped_duplicate")]
    ...
    if pending_items:
        ...
        for i, item in enumerate(pending_items[:5], 1):
            ...
            grp     = item.get("group_name") or item.get("hashtag", "")
    ...
    if pending:
        for g in pending[:3]:
            ...
            print(f"    • [{priv:7}] score={g.get('score','?'):>3}  members={mc:>8}  {g.get('name','?')[:40]}")
        if len(pending) > 3:
    ...
    if ERROR_LOG.exists():
        ...
        recent = [l for l in lines if today in l or (date.today() - timedelta(days=1)).isoformat() in l]
        if recent:
