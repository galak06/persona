"""Redis-backed task queue for DogFoodAndFun workers.
...
Queue names follow the pattern: ``dogfood:<worker>:tasks``
Dead-letter:                     ``dogfood:<worker>:dead``

Usage (producer)::

    from lib.task_queue import TaskQueue
    q = TaskQueue("fb-scan")
    q.push({"group_id": "123", "group_name": "Dog Food Lovers"})
...
class TaskQueue:
    def __init__(self, worker: str, brand: str = "dogfoodandfun") -> None:
        self.worker = worker
        self._key = f"{_NAMESPACE}:{brand}:{worker}:tasks"
        self._dead_key = f"{_NAMESPACE}:{brand}:{worker}:dead"
        self._r = _get_client()
