...
**Profile** — the canonical config for one platform identity (Facebook, Instagram, WordPress). Owns rate limits, schedule, capabilities, approval gates. Engine ships defaults at `social-automation/profiles/<platform>.json`; brand-specific overrides live in `<brand_dir>/brand.json` (slice C). Derived artifacts (`data/rate_limits.json`, `schedule.json`, plists) are regenerated by `python -m tools.profiles_build`.
