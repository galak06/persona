...
description: >
  Moderate pending comments on dogfoodandfun.com and post Nalla's Dad replies.
...
# WordPress Comment Handler — DogFoodAndFun
...
2. **Compose** (Claude drafts a Nalla's-Dad reply per pending item — see `## Drafting`).
...
```bash
cd /Users/gilcohen/Projects/dogfoodandfun/social-automation

# 1. Scan dogfoodandfun.com for held comments.
...
- **Voice is still Nalla's Dad** — same voice rules (`validate_voice` in `lib/comment_generator.py`) apply: specificity (Nalla/number/brand/timeframe), personal experience, end with a question.
...
- **Don't include links** to dogfoodandfun.com — reader is already on the site.
- **Length:** 80–300 chars (moderation replies are slightly longer than IG/FB because they can reference the post's content).

Example prompt shape:

```
You are Nalla's Dad replying to a comment on your own blog post.
...
natural. Mention Nalla or a concrete detail. End with a genuine follow-up
