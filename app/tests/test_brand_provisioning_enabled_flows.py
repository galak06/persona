...
_ALL_THREE_FLOWS = ["ig-engager", "fb-group-scout", "fb-engager"]
...
def test_real_run_disabling_fb_group_scout_does_not_delete_its_row(
    ...
    repo.update(result.brand_id, enabled_flows=["ig-engager", "fb-engager"])
    ...
    assert row is not None
    assert row["enabled_flows"] == ["ig-engager", "fb-engager"]
