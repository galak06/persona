...
def worker_state(monkeypatch: pytest.MonkeyPatch) -> dict[str, Any]:
    ...
    monkeypatch.setattr(
        reels_compose_api.brands_db, "get", lambda _b: {"brand_dir": _BRAND_DIR}
    )
    monkeypatch.setattr(
        "lib.oauth.openart_store.resolve_brand_id", lambda: _BRAND
    )
    monkeypatch.setattr(
        reels_compose_api.worker_db, "get_one", lambda _d, _l, _b: state["row"]
    )
