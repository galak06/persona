...
| Var | Purpose |
|---|---|
| `WP_URL` | e.g. `https://your-brand.com` |
...
| `RECIPE_REPORT_DIR` | `/mnt/persona` | where the daily report is written |
...
Add a plist under `~/Library/LaunchAgents/com.persona.recipe-publisher.plist` that calls `run_with_watchdog.py recipe-publisher` on your Sunday 09:00 IST schedule. The watchdog already handles:
...
- **Slide 1 (hero) — follow badge**: rounded-pill `@persona` in the top-right. First impression brands the account ID.
...
5. 8–12 hashtags including `#nallasdad` and `#persona`
