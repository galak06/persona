...
if TYPE_CHECKING:
    # Type-only: `from __future__ import annotations` means this name is
    # never evaluated at runtime, only referenced in annotations below.
    # A real (non-TYPE_CHECKING) import here pulls in generators.recipe ->
    # recipe_from_seed -> anthropic transitively -- a real dependency for
    # callers that pass an actual Recipe, but reel-publish callers pass a
    # duck-typed stub instead (see scripts.crewai_reels_pipeline), so this
    # module has no genuine runtime need for the real class. Confirmed
    # live: publishing a Reel from the containerized API failed with
    # ModuleNotFoundError: No module named 'anthropic' before this fix,
    # since that image never installs recipe-publisher/requirements.txt.
