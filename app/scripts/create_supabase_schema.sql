...
-- ────────────────────────────────────────────────────────────────────────────
-- oauth_tokens — Facebook / Instagram OAuth token storage
-- ────────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS oauth_tokens (
    id           TEXT        PRIMARY KEY DEFAULT gen_random_uuid()::text,
    brand_id     TEXT        NOT NULL,
    platform     TEXT        NOT NULL,           -- 'facebook' | 'instagram'
    token_type   TEXT        NOT NULL,           -- 'bearer' (user) | 'page'
    token_id     TEXT        NOT NULL DEFAULT '', -- page_id or ig_account_id
    access_token TEXT        NOT NULL,
    expires_at   TIMESTAMPTZ,                    -- NULL = non-expiring (page tokens)
    scope        JSONB       DEFAULT '[]',
    created_at   TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at   TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE INDEX IF NOT EXISTS idx_oauth_tokens_brand ON oauth_tokens(brand_id);
