...
Persona voice caption per group (category inferred from group name keywords),
...
Usage:
    python scripts/fb_group_post.py \\
        --url https://yourbrand.com/example-recipe-post/ \\
...
def draft_for_recipe(title: str, url: str) -> str:
    ...
    body = (
        f"Made a batch of {title} with {mascot} this weekend and they were gone "
        f"by Monday — been demanding them every morning since. "
...
def draft_for_running(title: str, url: str) -> str:
    mascot = settings.site.mascot_name
    body = (
        f"Been testing homemade training treats for trail runs with {mascot} — "
...
def draft_for_general(title: str, url: str) -> str:
    ...
    body = (
        ...
        f"Easy, pantry-friendly, and {mascot} approved."
    )
