"""Reference-library storage: paths, manifest, and candidate grouping.

Split out of `lib.crew.reference_library` (file-size discipline) as the layer
that knows where photos live and what the manifest says about them. Choosing
WHICH photo answers a request is the separate concern that stays there, and
imports run one way only: the resolver reads this module, never the reverse.

Everything here is re-exported by `lib.crew.reference_library`, so the ~33
existing importers keep their current import paths.
"""
