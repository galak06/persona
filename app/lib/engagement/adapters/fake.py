"""FakeAdapter — test double for OutboundAdapter (+ SupportsComment).

Replaces a real Playwright-backed adapter in scanner tests. Constructed with
canned sources + posts. Records every like() and comment() call so tests can
assert which posts were engaged with, and what text was posted.
"""
...
class FakeAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        comment_should_fail: bool = False,
        ...
        self._comment_should_fail = comment_should_fail
        ...
        # (post_id, text) for every comment() call the pipeline made.
    ...
    def comment(self, post: Post, text: str) -> CommentResult:
        """Satisfies `SupportsComment` so the pipeline's inline-comment path
        activates for this adapter (see `run_outbound_scan(inline_comment=)`)."""
        self.comments.append((post.post_id, text))
        if self._comment_should_fail:
            return CommentResult.failed("fake_failure")
