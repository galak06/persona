...
Return-value contract:
    - ``function == "main"`` (CLI entry point): the return value is a Unix
      exit code — ``0`` (or ``None``) is success, any non-zero return is a
      failure. Real hooks (e.g. ``scripts/publish_prepared.py::main``) are
      ``def main(argv=None) -> int`` run under ``sys.exit(main())``.
    - Otherwise: truthy return → success; ``None`` also counts as success.
"""
