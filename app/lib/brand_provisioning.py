...
# `api`'s brands root, and `app/brands/` already exists on disk (empty,
# provisioned ready) as of this PR. NOT a repo-root-of-the-outer-monorepo
# path -- `app/` is this engine's own repo root (the eventual extraction
# target for the open-source project described in the plan), so "sibling to
# the repo root" resolves to a sibling of `app/lib/`'s parent, i.e.
# `app/brands/`.
