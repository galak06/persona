...
description: >
  ...
  from .claude/state/comment_queue.json, drafts {{brand.persona}}-voice comments using
...
# Comment Composer — {{brand.name}}

Draft and post queued comments as {{brand.persona}}. Voice validation required before
...
# Skip own posts
if '{{brand.domain}}' in item.get('post_text', '').lower():
...
requires_approval = (
    ...
    '{{brand.domain}}' in draft.lower() or             # URL in comment
...
━━━━━━━━━━━━━━━━━━━━
Proposed comment:
"We went through this with {{brand.mascot}} last winter — switching to Open Farm
...
- `edit: We tried this with {{brand.mascot}} and...` → posts with new text (re-validates first)
