...
4. **Open Google Sheet:**
   ...
   - Extract all rows with columns: `Category`, `Topic`, `Target_Keyword`, `{{brand.mascot}}_Context`, `Post_Goal`, `Status`, `Input`
...
5. **Identify content gaps:**
   ...
   - What angle can {{brand.name}} uniquely address?
...
3. **Products to review:**
   ...
   - Mark if {{brand.name}} has affiliate links

4. **Internal link opportunities:**
   - Suggest 3-5 existing {{brand.name}} posts to link to
