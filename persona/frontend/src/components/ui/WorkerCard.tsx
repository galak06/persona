...
// eslint-disable-next-line react-refresh/only-export-components -- pure helper co-located with the component that uses it; dev-only HMR concern, no runtime impact
...
export function LogPanel({
  ...
  useEffect(() => {
    if (!open) return;
    // eslint-disable-next-line react-hooks/set-state-in-effect -- false positive: fetchLog's only setState calls (setLines/setError/setLoading) run after its internal `await`, i.e. in a later microtask, never synchronously within this effect's call stack
    void fetchLog();
