...
inputs:
  ...
  - name: dry_run
    required: false
    default: true
    description: When true, generates + writes a draft markdown report to /mnt/persona/ but does NOT publish to WP or IG. Default true for safety.
  - name: skip_ig
    required: false
    default: false
    description: Publish to WP only. Useful when IG token is mid-refresh.
trigger:
  - launchd: weekly, Sun 09:00 IST (com.persona.recipe-publisher.plist)
...
outputs:
  ...
  - Report at /mnt/persona/recipe-publisher-report-YYYY-MM-DD.md (always)
secrets_required:
  - WP_URL
  - WP_USER
  - WP_APP_PASSWORD
...
- Migration plan: `/mnt/persona/recipe-publisher-migration-plan.md`
- Audit handoff: `/mnt/persona/recipe-audit-handoff.md`
