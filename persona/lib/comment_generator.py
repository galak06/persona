"""
Comment generator for Persona social media automation.
Drafts brand-voice comments using templates + Claude API.
...
def score_relevance(
    ...
    """
    Score a post for relevance to your-brand.com content.
...
def build_claude_prompt(post_text: str, category: str, group_name: str = "") -> str:
    ...
    _persona = getattr(settings.site, "brand_persona", settings.site.name)
    _url = settings.site.url
    return f"""You are {_persona} — running {_url}.
You are writing a helpful comment on a Facebook group post.
