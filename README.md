...
- **Entrypoint:** `app/scripts/ig_scan.py`
- **Core:** `app/lib/engagement/pipeline.py` (orchestration), `app/lib/engagement/post_processor.py` (one-post visit), `app/lib/engagement/inline_comment.py` (agent draft + post)
- **Agent:** Gemini 2.5 Flash via `app/lib/gemini_client.py` — one structured call, self-approves (no human gate). Temp 0.7, `thinkingBudget=0`.
...
Each worker polls the DB independently, idempotent. Workers at `app/recipe-publisher/workers/`.

### Stack

```
app/lib/           Core library (engagement, config, rate limiter, dedup, OAuth, Gemini client)
app/scripts/       CLI entrypoints (ig_scan, fb_scan, fb_group_scout, publish_prepared, …)
app/api/           FastAPI on :5001 (brands, recipes, engagements, schedule, OAuth)
app/frontend/      React UI on :3000 (dashboard, recipes, groups, schedule, published)
app/recipe-publisher/  Recipe → social content pipeline (workers, templates, generators)
app/tests/         767 passing (pytest)
```
...
| Script | What it does |
|---|---|
| `app/scripts/publish_prepared.py` | Publish recipe → IG reel / FB reel / FB page post / Pinterest |
| `app/recipe-publisher/workers/worker_wp_pdf.py` | Recipe → WP draft + PDF card |
| `app/recipe-publisher/workers/worker_image.py` | Recipe → carousel slides + reel frames |
| `app/recipe-publisher/workers/worker_reel.py` | Recipe → compose MP4 reel |
| `app/recipe-publisher/workers/worker_publish.py` | Recipe → publish to social platforms |

### Content

| Script | What it does |
|---|---|
| `app/scripts/daily_wp_draft.py` | Generate daily WP draft |
| `app/scripts/refresh_keyword_research.py` | Keyword clustering |
| `app/scripts/refresh_trends_only.py` | Trend signals refresh |

### Operations

| Script | What it does |
|---|---|
| `app/scripts/onboard_brand.py` | Provision a new brand |
| `app/scripts/regenerate_plists.py` | Rebuild launchd plists from profiles |
| `app/scripts/status.py` | System health overview |

---

## Environment Variables

Key variables (full list in `.claude/settings.local.json` or `app/.env`):
